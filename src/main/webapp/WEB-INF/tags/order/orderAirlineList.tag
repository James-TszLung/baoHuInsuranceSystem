...
<tr>
	<td>${airBookLine.departure}-${airBookLine.arrival}&nbsp;${airBookLine.airlineCode}${airBookLine.flightNo}&nbsp;<b class="green_order">${airBookLine.cabin}</b><br /><strong><fmt:formatDate pattern="yyyy-MM-dd" value="${airBookLine.airDate}" />&nbsp;${airBookLine.departTime}</strong></td> 
</tr>
</c:forEach>
</table>
</c:if>