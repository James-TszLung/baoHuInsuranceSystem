...
<div class="content">
	<div class="ui_addUser">
		<form id="addform" enctype="multipart/form-data" autocomplete="off" action="/indemnity/editIndemnity.htm">
			...
			<div class="policy_added">
				<ul>
						...
						<span class="sel_span">
							<select name="dictionaryContentId">
								<c:forEach var="item" items="${typeLs}">
									<option value="${item.id}" ${sc.dictionaryContentId==item.id ? 'selected' : ''}>${item.content}</option>
								</c:forEach>
					         </select>
					    </span>
					</li>
					<li>
						<label>*保障类型：</label>
						<span class="sel_span"><input placeholder="请输入保障类型，最多50字" type="text" name="content" id="content" maxlength="50" value="${sc.content}"/></span>
					</li>
					<li>
						<label>排序：</label>
						<span class="sel_span"><input type="number" name="sort" value="${sc.sort}"/></span>
					</li>
					<li>
						<label>子项目：</label>
						<span class="sel_span">
							<table>
								<tr>
									<td id="fwphtd">
										...
										<c:forEach var="item" items="${sc.indemnitySubLs}" varStatus="vs">
                                          <div  id='qtdiv${vs.index}' class="qtdiv" name="qtdiv" index='${vs.index}' style="width:100%;overflow:hidden;">
											  <input name="indemnitySubLs[${vs.index}].id" type="hidden" value="${item.id}"/>
											<span class="form-block" >
												<input name="indemnitySubLs[${vs.index}].name" maxlength="50" type="text" value="${item.name}"/> : <input name="indemnitySubLs[${vs.index}].sort" value="${item.sort}" type="number">
											</span>
											&nbsp&nbsp
											<a onclick="delqt('${vs.index}')" style="color:#2A9CD5">删除</a>
										  </div>&nbsp
										</c:forEach>
									</td>
								</tr>
