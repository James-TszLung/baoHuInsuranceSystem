...
<%--		<form id="addform" enctype="multipart/form-data" autocomplete="off">--%>
	<form id="addform" autocomplete="off" method="post">
			...
			<div class="policy_added">
				<ul>
					<li>
						<label>*字段名称：</label>
						<span class="sel_span">
							<input placeholder="请输入字段名称" type="text" name="name" id="name" value="${sc.name}"/>
						</span>
					</li>
							...
							<table>
								<tr>
									<td id="fwphtd">
										...
										<c:forEach items="${sc.contentLs}" var="ls" varStatus="status">  
											<div  id='qtdiv${status.count-1}' class="qtdiv" name="qtdiv" index='${status.count-1}' style="width:100%;overflow:hidden;">
												<input name='contentLs[${status.count-1}].id' value="${ls.id}" type="hidden" />
												<span class="form-block" >
													<input name="contentLs[${status.count-1}].content" type="text" value="${ls.content}" /> : <input name="contentLs[${status.count-1}].sort"  value="${ls.sort}" type="number">
												</span>
												&nbsp&nbsp
												<a onclick="delqt('${status.count-1}')" style="color:#2A9CD5">删除</a>
											</div>&nbsp
										</c:forEach>
									</td>
								</tr>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/scripts/unique/dictionary/editDictionary_jquery.js"></script>
