(function($){
    $.querycity = function(input,options){
        var input = $(input);
        var hideInput = $('#'+options.hiddenInput);
        ...
        $('.cityquery_box').append("<div id='pop_city_"+input.attr('id')+"' class='pop_city' style='display:none'><p class='pop_head'></p><ul class='list_label'></ul><div class='pop_city_container'></div></div>");
        $('.cityquery_box').append("<div id='suggest_city_"+input.attr('id')+"' class='list_city' style='display:none'><div class='list_city_head'></div><div class='list_city_container'></div><div class='page_break'></div></div>");
        var popMain = $("#pop_city_"+input.attr('id'))
        var popBtn;
        if(options.listBtn)
        	popBtn = $("#"+options.listBtn);
        ...
        popInit();
        var popCloseBtn = popMain.find('.list_city_closebtn');
        var sugCloseBtn = suggestMain.find('.list_city_closebtn');
        resetPosition(); 	    
        ...
        sugCloseBtn.on('click',function(){
        	suggestMain.hide();
        });
        
        var inputClickFunc = function(){		
        ...
        input.focus(function(){
		    ...
		    popMain.show();
		    resetPosition();
        }).click(inputClickFunc).blur(function(){				
	    ...
	    popContainer.find('a').bind('click',function(){
		    input.val($(this).html() + "（" + ($(this).attr("code")).toUpperCase() + "）");
		    hideInput.val(($(this).attr("code")).toUpperCase());
		    popMain.hide();
	    });
            ...
            if(13 == keyCode){
                if(suggestMain.find('.list_city_container a.selected').length > 0){
                    input.val(suggestMain.find('.list_city_container a.selected').children('b').text()
                    		+"（"+((suggestMain.find('.list_city_container a.selected').children('b')).attr('code')).toUpperCase()+"）");
                    hideInput.val(((suggestMain.find('.list_city_container a.selected').children('b')).attr('code')).toUpperCase());
                    suggestMain.hide();
                }
            }
        ...
        bindSuggestFun();
        function bindSuggestFun(){
        	suggestMain.find('.list_city_container a').bind('click',function(){
                input.val($(this).children('b').text()+"（"+($(this).children('b').attr('code')).toUpperCase()+"）");
                hideInput.val(($(this).children('b').attr('code')).toUpperCase());
                suggestMain.hide();
...
;
		    if(isHave){
                ...
                var _data= _tmp[item];
                city_container.append("<a href='javascript:void(0)' style='display:none'><span>"+_data[2]+"</span><b code='"+  _data[0] +"'>"+     _data[1] +"</b></a>");                
                }
			    suggestMain.find('.list_city_head').html(value+",按拼音排序<div class='list_city_closebtn'>X</div>");
                setAddPage(1);
                setTopSelect()
    		}else{
	    		suggestMain.find('.list_city_head').html("<span class='msg'>对不起,找不到"+value+"</span><div class='list_city_closebtn'>X</div>");
		    }
            suggestMain.show();
            bindSuggestFun();
	    }
        ...
        function setAddPageHtml(pageIndex){
            ...
            if(suggestMain.find('.list_city_container').children().length > options.suggestLength){
			    ...
			    if (pageIndex<pageBreakSize) {
                    cityPageBreak.append("<a href='javascript:void(0);' inum='"+ (parseInt(pageIndex, 0)+1) +"'>-&gt;</a>");
                }
                cityPageBreak.show();           
            }else{
                cityPageBreak.hide();    
        ...
        function popInit(){
            var index = 0;
            popMain.find('.pop_head').html(options.popTitleText+"<div class='list_city_closebtn'>X</div>");
            if(!options.tabs){
                popContainer.append("<ul id='label_"+input.attr('id')+"_container' class='current'></ul>");
                labelMain.remove();
                for( var item in options.data){
                      $("#label_"+input.attr('id')+"_container").append("<li><a href='javascript:void(0)' code='"+options.data[item][0]+"'>"+options.data[item][1]+"</a></li>");
                }
                return;
            }
    		for(var itemLabel in options.tabs){		
			    ...
			    for(var item in options.tabs[itemLabel]){
				    ...
				    $("#label_"+input.attr('id')+index+"_container").append("<li><a href='javascript:void(0)' code='"+options.data[cityCode][0]+"'>"+options.data[cityCode][1]+"</a></li>");
			    }
		    }			
        }
        function resetPosition(){
        	...
        	if(parent && parent.prop('class')=='cityquery_box')
        		_target = parent;
            popMain.css('top',_target.position().top+_target.outerHeight()).css('left',_target.position().left);
            suggestMain.css('top',_target.position().top+_target.outerHeight()).css('left',_target.position().left);
        }
    }
