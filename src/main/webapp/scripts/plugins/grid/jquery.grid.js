...
;(function ($) {
    ...
    $.grid.prototype = {
        ...
        _create: function () {
            ...
            html.push(self._createShowColMenu());
            ...
            opt.toolBar && html.push('<tr><td style="padding:0"><div class="grid-title"><div class="grid-toolbar-right"><font face="webdings">a</font><a href="#" class="grid-show-col">展示列</a></div></div></td></tr>');
            ...
            if (opt.showFooter) {
                ...
                //分页
                html.push('<div class="grid-pager" style="height:30px;"><a href="#" rel="start" class="grid-pagenav disabled">首页</a><a href="#" rel="prev" class="grid-pagenav disabled">上一页</a>');
                ...
                //统计
                html.push('<div class="grid-counter" style="height:30px;">第<span>1</span>页, <span>0</span>条记录 / 共<span>0</span>条记录</div>');
                html.push('</div></td></tr>');
            }
        ...
        //展示列菜单-创建
        _createShowColMenu: function(){
        	var cols = this.options.columns;
        	var menuHtml = '<div class="grid-col-menu" align="center"><table class="grid-col-table" cellpadding="0" cellspacing="0"><tr class="grid-col-table-head"><td valign="middle">列</td><td></td></tr>';
        	var t = 0;
        	for(var i=0; i<cols.length; i++){
        		if(cols[i].field != ('rowIndex'||'checkbox')
        				&& t%2==0)
        			menuHtml += '<tr>';
        		switch (cols[i].field) {
				case 'rowIndex':
				...
				case 'checkbox':
					break;
				default:
					var checked = cols[i].hidden ? '':'checked';
					menuHtml += '<td align="left"><label><input type="checkbox" class="show-col-cb" value="'+i+'" '+checked+'>'+cols[i].title+'</label></td>';
					break;
				}
        		t++;
        		if(t%2==0)
        ...
        _init: function () {
            ...
            $(document).click(function (e) {
                var m = $(".grid-col-menu"),//菜单
                    mu = $(".grid-col-menu")[0],
                    mb = $(".grid-show-col")[0],//弹出按钮
                    target = e.target;
                if (mu !== target && !$.contains(mu, target)
                		&& mb != target && !$.contains(mb, target)) {
                    m.hide();
                }
            });
            ...
            //弹出显示隐藏列菜单
            self.el.on('click.grid','a.grid-show-col', function(){
            	//var defaultAlign = "Bottom";
            	var mgap = 3;//按钮菜单间隔
            	var menu = $(".grid-col-menu");
            	var offsetR,offsetB,
            		tHeight = $(this).height(),//按钮高
            		tWidth = $(this).width();//按钮宽
            	if(menu.css("display")=='block'){
    	        	...
    	        	offsetR = $(document).width()-x;
    	        	offsetB = $(window).height()-y;
    	        	if(x < menu.width()){//出现在右侧
    	        		menu.css("left", x);
    	        		if(y <= menu.height()){//出现在下侧
    	    	        	menu.css("top", y + tHeight + mgap);
    	        		}else if(offsetB <= menu.height()){//出现在上侧
    	    	        	menu.css("top", y - menu.height() - mgap);
    	        		}else{//默认下侧（以后会有中侧）
    	    	        	menu.css("top", y + tHeight + mgap);
    	        		}
    	        	}else if(offsetR < menu.width()){//出现在左侧
    	        		menu.css("left", x - (menu.width() - tWidth));
    	        		if(y <= menu.height()){//出现在下侧
    	        			menu.css("top", y + tHeight + mgap);
    	        		}else if(offsetB <= menu.height()){//出现在上侧
    	        			menu.css("top", y - menu.height() - mgap);
            ...
            //显示/隐藏列
            self.el.on('click.grid','input.show-col-cb',function(){
            	var _this = $(this);
            	var check = _this.prop('checked');
            	var _index = _this.val();
            	//var _dataGridRows = "#" + opt.container.id + " table tr";
            	var _dataGridRows = ".grid-head tr,.grid-data tr"
            	if(check){
            		$(_dataGridRows).each(function() {  
            	        $(this).find("th:eq(" + _index + ")").show();  
            	        $(this).find("td:eq(" + _index + ")").show();  
            	    })
            	}else{
            		$(_dataGridRows).each(function() {  
            			$(this).find("th:eq(" + _index + ")").hide();  
            	        $(this).find("td:eq(" + _index + ")").hide();  
            	    })
            	}
            ...
            if (opt.checkbox) { //复选框
                ...
                self.el.on('click.grid', 'input.checkAll', function (e) {
                    ...
                    self.dataContainer.find(':checkbox').each(function (i) {
                    	if($(this).attr('disabled')!='disabled'){//如果checkbox不可用，则全选也同样不能被选择
                ...
                self.gridData.on('click.grid', 'tr', function (e) {
                    ...
                    if(_this.find(':checkbox').attr('disabled')!='disabled'){//如果checkbox不可用，则点击行也同样不能被选择
            ...
            if (msg || !data.length) {
                ...
                for (; i < dlen; i++) {
                    ...
                    for (; j < clen; ) {
                        col = dataCols[j++];
                        field = col.field;
                        html.push('<td', (col.align ?  ' align=' + col.align : ''), (col.hidden ? ' style=\"display: none;\"' : ''), '>');
        ...
        _onComplete: function (e) {
            this.gridSizer && this.gridSizer.children().height("100%");//以下2行解决：IE6绝对定位的元素高度必须给个具体的值   100px 暂定修改方法
            this.gridLoading.height = this.el.height() - 10 + 'px';
        ...
        _setHead: function (headCols, headRowCount) {
            var html = [], count = headRowCount, i = 0, len = headCols.length,
                col, row, colspan, rowspan, align, field, cls1, cls2, hidden;
            if (count === 1) {
                html.push('<tr>');
                for (; i < len; ) {
                    col = headCols[i++];
					...
					hidden = col.hidden ? ' style=\"display: none;\"' : '';
                    html.push('<td', field, align , hidden ,'><span', cls2, '>', col.title, '</span>', '</td>');
                }
 ...
 * Bug 如果父级层初始为 display:none;就获取不到偏移位置
 */