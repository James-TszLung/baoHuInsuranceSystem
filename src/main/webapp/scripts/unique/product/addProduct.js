$(function () {
	...
	// init();
	bindEvent();
	$('#insuranceTypeId').children().eq(0).trigger('change');
	loadAcBox();
})
...
function bindEvent() {
	...
	$("#addBtn").click( function() {
		...
		$("#addform").form("submit", {
			url : ctxPath +"/product/saveProduct.htm",
	...
	$("#insuranceTypeId").change(function(){
		...
		if(calculation!=undefined && calculation!=null && calculation!="") {
			...
			if(calculations.length>0){
				...
				str += thStr;
				str += '</tr></thead><tbody class="table-list"><tr class="table-item caculate-item" indexItem="0">';
				str += tdStr;
				str += '</tr></tbody>';
	...
	$('#modalGuardWhatConfirm').on('click', function (event) {
		...
		relationHtml += '<td><input type="hidden" name="guaranteeRelationLs['+num+'].dictionaryContentId" type="hidden" value="'+guaranteeTypeId+'"/>' +
	...
	$('#modalGuardBuyRuleConfirm').on('click', function (event) {
		...
		var ruleId = modal.find('.guard-buy-rule-title').find("option:selected").attr("ruleId");
		if(title==""){
			alert("规则类别不能为空");
			return;
		}
		...
		template.find('.guard-buy-rule-id').val(ruleId);
		template.find('.btn-edit-item').attr('data-content', content);
		template.find('.btn-edit-item').attr('data-title', title);
		template.find('.btn-edit-item').attr('data-id', ruleId);
		template.find('.guard-buy-dictionaryContentId').val(ruleId);
