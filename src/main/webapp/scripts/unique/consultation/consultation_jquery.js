...
var lastPostData;
function doSearch(f) {
	var searchURL = "/operate/serachConsultation.htm";
	if (f) {
...
// 切换到添加菜单页面
function reply(id) {
	visitUrl(ctxPath + "/operate/replyConsultationPage.htm?id=" + id, 1, 1, 1);
}
//切换到添加菜单页面
function detail(id) {
	visitUrl(ctxPath + "/operate/goReplyConsultationDetail.htm?id=" + id, 1, 1, 1);
}
