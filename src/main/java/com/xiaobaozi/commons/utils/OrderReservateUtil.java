...
import com.skyecho.commons.Constants;
import com.skyecho.shenzhenairlines.baseinfo.vo.Airlines;
import com.skyecho.shenzhenairlines.baseinfo.vo.Iata;
...
public class OrderReservateUtil {
	...
	*//**
	...
	public String doPatBySegmentAndCustNoXml(String Airline, String FlightNo,
		...
		String result = "";
		try {
			result = HttpUtil.http(url, param.toString());
		} catch (Exception e) {
	...
	*//**
	 ...
	 * @return
	 *//*
	public String doPatByPnrAndCustNoXml(String pnrNo, String passengerType,
	...
	*//**
	 ...
	 * @return
	 *//*
	public static Object parseFlightsToCN(Object obj){
		if(obj instanceof List){
			List list = (List)obj;
			for(int i=0; i<list.size(); i++){
				list.set(i, parseFlightToCN(list.get(i)));
			}
			return list;
		} else if(obj instanceof Map) {
			return parseFlightToCN(obj);
		}
		return null;
	}
	
	@SuppressWarnings("unchecked")
	public static Map parseFlightToCN(Object fObject){
		Map fm = (Map)fObject;
		List<Object> alList = Constants.cacheListByMap.get(Airlines.class.getName());
		if(alList!=null){
			for(Object alObj:alList){
				Airlines al = (Airlines)alObj;
				if(al.getAirlineCode().equals(fm.get("AIRCODE"))
						|| al.getAirlineCode().equals(fm.get("airlineCode"))){
					fm.put("CAIRCODE", al.getAirlineCname());
				}
			}
		}
		List<Object> iataList = Constants.cacheListByMap.get(Iata.class.getName());
		if(iataList!=null){
			for(Object itObj:iataList){
				Iata it = (Iata)itObj;
				if(it.getIataID().equalsIgnoreCase((String)fm.get("ARRIVECITY"))
						|| it.getIataID().equalsIgnoreCase((String)fm.get("arrival"))){
					fm.put("CARRIVE_AIRPORT", it.getAirportCname());
					fm.put("CARRIVECITY", it.getCityCname());
				}
				if(it.getIataID().equalsIgnoreCase((String)fm.get("DEPARTCITY"))
						|| it.getIataID().equalsIgnoreCase((String)fm.get("departure"))){
					fm.put("CDEPART_AIRPORT", it.getAirportCname());
					fm.put("CDEPARTCITY", it.getCityCname());
				}
			}
		}
		return fm;
